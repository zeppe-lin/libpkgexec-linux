...
grep -F "version: '0.6.1'" "$root/meson.build" >/dev/null || fail 'project version is not 0.6.1'
...
grep -F '## libpkgexec-linux 0.6.1' "$root/HISTORY.md" >/dev/null || fail '0.6.1 history entry is absent'
