...
grep -F "version: '0.6.2'" "$root/meson.build" >/dev/null || fail 'project version is not 0.6.2'
...
printf '%s\n' "$block" | grep -F "  version: ['>=2.1.1', '<3.0.0']," >/dev/null ||
  fail 'execution dependency interval is not >=2.1.1,<3.0.0'
grep -F '## libpkgexec-linux 0.6.2' "$root/HISTORY.md" >/dev/null || fail '0.6.2 history entry is absent'
