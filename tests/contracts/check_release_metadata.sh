...
grep -F "version: '0.7.0'" "$root/meson.build" >/dev/null || fail 'project version is not 0.7.0'
...
grep -F '## libpkgexec-linux 0.7.0' "$root/HISTORY.md" >/dev/null || fail '0.7.0 history entry is absent'
